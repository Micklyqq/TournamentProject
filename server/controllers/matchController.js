...
class MatchController {
  ...
  checkAndHandleNextRound = async (req, res) => {
    try {
      ...
      const completedMatches = matches.filter(
        (match) => match.winnerTeamId !== null && match.grid === "startBracket"
      );
      if (completedMatches.length === tournament.size / 2) {
        ...
        matches.forEach((match) => {
          if (match.winnerTeamId) {
            ...
            losers.push(
              match.teams.find((team) => team.id !== match.winnerTeamId).id
            );
          }
        });

        await Match.update(
          { grid: "startBracketEnd" },
          {
            where: {
              tournamentId,
              grid: "startBracket",
        ...
        const winnerBracket = await this.createTournamentBracket(
          tournamentId,
          winners,
          "winners",
          1
        );
        const loserBracket = await this.createTournamentBracket(
          tournamentId,
          losers,
          "losers",
          1
        ...
        const sortLosersLastRound = sortedMatches.filter(
          (match) => match.grid === "loserBracket"
        );
        const findLosersLastRound =
          sortLosersLastRound[sortLosersLastRound.length - 1].round;

        const sortWinnersLastRound = sortedMatches.filter(
          (match) => match.grid === "winnerBracket"
        );
        const findWinnersLastRound =
          sortWinnersLastRound[sortWinnersLastRound.length - 1].round;

        const winnerCurrentRound = matches.filter(
          (match) =>
            match.grid === "winnerBracket" &&
            match.round === findWinnersLastRound
        );
        const winnerCompleted = matches.filter(
          (match) =>
        ...
        const loserCurrentRound = matches.filter(
          (match) =>
            match.grid === "loserBracket" && match.round === findLosersLastRound
        );
        const loserCompleted = matches.filter(
          (match) =>
            match.winnerTeamId !== null &&
            match.grid === "loserBracket" &&
            match.round === findLosersLastRound
        );
        ...
        if (finalRound.length > 0) {
          teamMatch = await MatchTeam.findAll({
            where: { matchId: finalRound[0].id },
          });
        ...
        let finalBracket = [];
        if (
          winnerCurrentRound.length === winnerCompleted.length &&
          winnerCompleted.length > 1 &&
          finalRound.length === 0
        ) {
          const winners = [];
          ...
          winnerBracket = await this.createTournamentBracket(
            tournamentId,
            winners,
            "winners",
            findWinnersLastRound + 1
          );
        }

        if (
          loserCurrentRound.length === loserCompleted.length &&
          loserCompleted.length > 1 &&
          finalRound.length === 0
        ) {
          const losers = [];
          ...
          loserBracket = await this.createTournamentBracket(
            tournamentId,
            losers,
            "losers",
            findLosersLastRound + 1
          );
        }
        ...
        if (
          ...
          loserCompleted.length === 1 &&
          finalRound.length < 3 &&
          firstFinalTeamWins < 2 &&
          secondFinalTeamWins < 2
        ) {
          const final = [
            winnerCompleted[0].winnerTeamId,
            loserCompleted[0].winnerTeamId,
          ];
          finalBracket = await this.createTournamentBracket(
            tournamentId,
            final,
            "final",
            1
        ...
        if (firstFinalTeamWins === 2 && tournament.teamId === null) {
          console.log("FDSFSFSDFSDFSDFSDFSFSDFSDFSFSDFSDF");
          await Tournament.update(
            { teamId: firstFinalTeam, userId: null, status: true },
            { where: { id: tournamentId } }
          );
        }
        if (secondFinalTeamWins === 2 && tournament.teamId === null) {
          console.log("DFFFFFFFFFDSFSF");
          await Tournament.update(
            { teamId: secondFinalTeam, userId: null, status: true },
            { where: { id: tournamentId } }
          );
        }
  ...
  create = async (req, res) => {
    ...
    try {
      const createdMatches = await this.createTournamentBracket(
        tournamentId,
        teams,
        "start"
      );
  ...
  async getAllMatchesForTournament(req, res) {
    try {
      ...
      throw new Error(
        `Ошибка при получении матчей для турнира: ${error.message}`
      );
    }
  }
